...
#    READY_table = [b'+',b'R',b'E',b'A',b'D',b'Y'] # if RESET received, state_table = READY_table
    ...
    async def xcvr(self, scr : _curses.window) -> None:
        ...
        if self.urwid:
            import urwid
            # I have to create my own class, sorry
            class Border(urwid.Widget):
                _sizing = frozenset(['box'])

                def render(self, size, focus=False):
                    (maxcol, maxrow) = size
                    return urwid.TextCanvas(text=[' '.encode('utf-8')*maxcol]*maxrow, maxcol=maxcol,)

            widget = Border()
            #widget.render((dsply.MAX_COL, dsply.MAX_ROW))

            eloop = urwid.AsyncioEventLoop(loop=asyncio.get_event_loop())
            self.uloop = urwid.MainLoop(widget, event_loop=eloop)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--urwid', action='store_true', help='Enable urwid GUI. Experimental!')
