...
from src.ui.constants import ColorPair, WindowSize, BorderPos
...
def initialize_display(stdscr) -> Tuple[curses.window, StatusWindow, ReceiveWindow, TransmitWindow]:
    ...
    try:
        ...
        border_win = curses.newwin(WindowSize.MAX_ROW, WindowSize.MAX_COL, 0, 0)
        border_win.border()
        ...
        for y in [BorderPos.RX_ST_DIV, BorderPos.ST_TX_DIV]:
            ...
            border_win.hline(y, 1, curses.ACS_HLINE, WindowSize.MAX_COL-2)
            border_win.addch(y, WindowSize.MAX_COL-1, curses.ACS_RTEE)
